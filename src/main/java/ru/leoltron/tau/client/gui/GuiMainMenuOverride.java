...
public class GuiMainMenuOverride extends GuiMainMenu {
	...
	public void initGui()
    {
        ...
        {
            this.splashText =  I18n.format("splash.happyBirthday")+", Leolt!";
        }
        else if (calendar.get(2) + 1 == 9 && calendar.get(5) == 7)
        {
            this.splashText =  I18n.format("splash.happyBirthday")+", SkyStrider!";
        }
    ...
    {
    	boolean isLegalRP = true;
    	/*List<ResourcePackRepository.Entry> rps =Minecraft.getMinecraft().getResourcePackRepository().getRepositoryEntries();
		for(ResourcePackRepository.Entry rpe : rps){
			if(rpe.getResourcePack().hashCode() != DzetaModpack.hash){
				isLegalRP = false; 
				break;
			}
		}*/
    	GuiButton singleplayerButton = new GuiButton(1, this.width / 2 - 100, p_73969_1_, I18n.format("menu.singleplayer"));
        this.buttonList.add(singleplayerButton);
        GuiButton connectButton = new GuiButton(2, this.width / 2 - 100, p_73969_1_ + p_73969_2_ * 1, I18n.format("menu.connectTau"));
        connectButton.enabled = isLegalRP;
        this.buttonList.add(connectButton);
        GuiButton websiteButton = new GuiButton(14, this.width / 2 - 100, p_73969_1_ + p_73969_2_ * 2, I18n.format("gui.menu.website"));
        GuiButton forumButton = new GuiButton(6, this.width / 2 - 100, p_73969_1_ + p_73969_2_ * 2, I18n.format("gui.menu.forum"));
        forumButton.xPosition = this.width / 2 + 2;
        websiteButton.width = 98;
        forumButton.width = 98;
        this.buttonList.add(websiteButton);
        this.buttonList.add(forumButton);
    }
	...
	protected void actionPerformed(GuiButton button)
    {
        ...
        if (button.id == 2)
        {
        	this.mc.displayGuiScreen(new GuiConnecting(this, mc, "ru-craft.net", 25000));
        }
        ...
        if (button.id == 14)
        {
        	    try
                {
                    ...
                    oclass.getMethod("browse", new Class[] {URI.class}).invoke(object, new URI(websiteUrl));
                }
        ...
        if (button.id == 6)
        {
        	try
            {
                Class oclass = Class.forName("java.awt.Desktop");
                Object object = oclass.getMethod("getDesktop", new Class[0]).invoke(null);
                oclass.getMethod("browse", new Class[] {URI.class}).invoke(object, new URI(websiteUrl+"forum/index.php"));
            }
            catch (Throwable throwable)
            {
                logger.error("Couldn\'t open link", throwable);
